...
public class AddressPicker extends LinkagePicker {
    ...
    //省市区数据
    private ArrayList<Province.DataBean> provinces = new ArrayList<Province.DataBean>();

    public AddressPicker(Activity activity, ArrayList<Province.DataBean> provinces) {
    ...
    public Province.DataBean getSelectedProvince() {
        return provinces.get(selectedFirstIndex);
    }

    public Province.DataBean.YearBean getSelectedCity() {
        if (getSelectedProvince().getYear()!=null) {
            return getSelectedProvince().getYear().get(selectedSecondIndex);
        }else {
            return null;
        }
    }

    public Province.DataBean.YearBean.MasterBean getSelectedCounty() {
        if (getSelectedCity()!=null) {
            if (getSelectedCity().getMaster() != null) {
                return getSelectedCity().getMaster().get(selectedThirdIndex);
            } else {
                return null;
            }
        }else {
...
//                    }
    ...
    public void onSubmit() {
        ...
        if (onLinkageListener != null) {
            Province.DataBean province = getSelectedProvince();
            Province.DataBean.YearBean city = getSelectedCity();
            Province.DataBean.YearBean.MasterBean county = null;
            if (!hideCounty) {
    ...
    public static class AddressProvider implements DataProvider {
        ...
        public AddressProvider(List<Province.DataBean> provinces) {
            parseData(provinces);
        }
        ...
        private void parseData(List<Province.DataBean> data) {
            int provinceSize = data.size();
            int countySize;
            int citySize;
            //添加省
            for (int x = 0; x < provinceSize; x++) {
                Province.DataBean  pro = data.get(x);
                firstList.add(pro.getName());
                List<Province.DataBean.YearBean> cities = pro.getYear();
                ...
                for (int y = 0; y < citySize; y++) {
                    Province.DataBean.YearBean cit = cities.get(y);
                    xCities.add(cit.getValue());
                    List<Province.DataBean.YearBean.MasterBean> counties = cit.getMaster();
                    ...
                    //添加区县
                    if (countySize == 0) {
                        yCounties.add(cit.getValue());
                    } else {
                        for (int z = 0; z < countySize; z++) {
                            Province.DataBean.YearBean.MasterBean cou = counties.get(z);
                            yCounties.add(cou.getValue());
                        }
                    }
