 ...
 * @see County
 */
public class AddressPicker2 extends LinkagePicker {
    private OnLinkageListener2 onLinkageListener;
    ...
    //省市区数据
    private ArrayList<Province2.DataBean> provinces = new ArrayList<Province2.DataBean>();

    public AddressPicker2(Activity activity, ArrayList<Province2.DataBean> provinces) {
    ...
    public Province2.DataBean getSelectedProvince() {
        return provinces.get(selectedFirstIndex);
    }

    public Province2.DataBean.ChildsBeanX getSelectedCity() {
        if (getSelectedProvince().getChilds()!=null) {
            return getSelectedProvince().getChilds().get(selectedSecondIndex);
        }else {
            return null;
        }
    }

    public Province2.DataBean.ChildsBeanX.ChildsBean getSelectedCounty() {
        if (getSelectedCity()!=null) {
            if (getSelectedCity().getChilds() != null) {
                return getSelectedCity().getChilds().get(selectedThirdIndex);
            } else {
                return null;
            }
        }else {
    ...
    public void setOnLinkageListener(OnLinkageListener2 listener) {
...
//                    }
    ...
    public void onSubmit() {
        onLinkageListener.onSubmit();
        if (onLinkageListener != null) {
            Province2.DataBean province = getSelectedProvince();
            Province2.DataBean.ChildsBeanX city = getSelectedCity();
            Province2.DataBean.ChildsBeanX.ChildsBean county = null;
            if (!hideCounty) {
    ...
    public static class AddressProvider implements DataProvider {
        ...
        public AddressProvider(List<Province2.DataBean> provinces) {
            parseData(provinces);
        }
        ...
        private void parseData(List<Province2.DataBean> data) {
            int provinceSize = data.size();
            int countySize;
            int citySize;
            //添加省
            for (int x = 0; x < provinceSize; x++) {
                Province2.DataBean  pro = data.get(x);
                firstList.add(pro.getName());
                List<Province2.DataBean.ChildsBeanX> cities = pro.getChilds();
                ...
                for (int y = 0; y < citySize; y++) {
                    Province2.DataBean.ChildsBeanX cit = cities.get(y);
                    xCities.add(cit.getName());
                    List<Province2.DataBean.ChildsBeanX.ChildsBean> counties = cit.getChilds();
                    ...
                    if (countySize == 0) {
                        ...
                        for (int z = 0; z < countySize; z++) {
                            Province2.DataBean.ChildsBeanX.ChildsBean cou = counties.get(z);
                            yCounties.add(cou.getName());
                        }
                    }
